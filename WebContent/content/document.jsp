						...
						<div class="collapse navbar-collapse " id="navbar-collapse-1">
							<div class="header-nav">
								<ul class="nav navbar-nav">
									...
									<li class="active"><a href="document.html">文档</a></li>
								</ul>
							</div><!-- header-nav结束 -->	
							...
							<form action="#" class="form-inline header-search" methed="post" id="search">
								...
								<div class="form-group">
									<input type="text" class="form-control " placeholder="视频/音频/图像/文档"  size="20" id="searchText" style="width:220px;"/>
...
<!--         </div>
    ...
    <script type="text/javascript">
		...
		function getDocumentDataByAjax(url){
			$.getJSON(url, function(data){
				for(var i = 0; i < 10; i++){
					$('.changelist').append(
						"<li class='list-1-o'>"
                        +   "<div class='lesson-info'>"
                        +        "<a href='javascript:void(0)' onclick='forwadToViewPage("+data.list[i].resourceId+")'><img src='img/lesson.png' style='width: 200px'></a>"
                        +       "<span style='color:#777;'>"+上传者+
                        +        "<span class='word-name'>"+data.list[i].userName+"</span>"
                        +        "<span class='word-time' style=''>"+data.list[i].resourceUploadTime+"</span>"
                        +       "</span>"
                        +       "<div>"
                        +       "<p class='txt-style' name='txt-style'>"+data.list[i].resourceName+"</p>"
                        +       "</div>"
                        +   "</div>"
                        +"</li>"
					);
		...
		getDocumentDataByAjax("getDocumentAction.action");
		
//         $(function() {  
//             $('.media').media({width:467, height:606});  
//         });  


        function forwadToViewPage(id){
            window.location.href+"DocumentAction.action?resourceId=id";
        }
